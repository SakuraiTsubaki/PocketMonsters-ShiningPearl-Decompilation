...
if command -v apt-get >/dev/null 2>&1; then
  ...
  DEBIAN_FRONTEND=noninteractive "${SUDO[@]}" apt-get install -y --no-install-recommends ca-certificates curl git make cmake ninja-build python3 build-essential llvm clang lld || true
fi
if [[ ! -x "$BIN_DIR/hactool" ]]; then
  rm -rf "$SRC_DIR/hactool"
  git clone --depth 1 https://github.com/SciresM/hactool.git "$SRC_DIR/hactool"
  make -C "$SRC_DIR/hactool" -j"$(getconf _NPROCESSORS_ONLN 2>/dev/null || echo 2)"
  ln -sfn "$SRC_DIR/hactool/hactool" "$BIN_DIR/hactool"
fi
JSON_FILE="$(mktemp)"; trap 'rm -f "$JSON_FILE"' EXIT
if curl -fsSL https://nightly.eden-emu.dev/latest/release.json -o "$JSON_FILE"; then
  URL="$(python3 - "$JSON_FILE" <<'PY'
import json,sys
...
PY
  ...
  if [[ -n "$URL" && ! -x "$EMU_DIR/Eden.AppImage" ]]; then curl -fL --retry 3 "$URL" -o "$EMU_DIR/Eden.AppImage" && chmod +x "$EMU_DIR/Eden.AppImage"; fi
fi
